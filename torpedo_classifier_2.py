...
test_set = TorpedoDataset('test2.csv')
test_loader = DataLoader(dataset=test_set, batch_size=600, shuffle=False)
...
if __name__ == '__main__':
    ...
    """
    # classifier = RNNClassifier(input_size=2, hidden_size=100, output_size=n_class).to(device)
    # classifier.load_state_dict(torch.load('/data2/home/chaoqun/simulator/model/model_save'))
    # acc_list = test_part_model()
    # acc_max = test_model()
    # acc_list.append(acc_max)
    # epoch = np.arange(0, (len(acc_list))*10, 10)
    # acc_list = np.array(acc_list)
    # plt.plot(epoch, acc_list)
    # ax = plt.gca()
    # ax.locator_params('x', nbins=10)
    # plt.xlabel('From_1_To_number,1-10,1-20,...,1-Max')
    # plt.ylabel('Accuracy%')
    # plt.grid()
    # plt.show()
